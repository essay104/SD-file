...
const addCart = (e) => {
  ...
  if (checked) {
    ...
    selected = selected.filter((product) => {
      return product.id !== parsenInt(id);
    });
  }
  const updateTotal = (price) => {
    const taotalprice = document.querySelector(".total-price");
    const formatted = new Intl.NumverFormat("ko-kr", {
      style: "currency",
      currency: "KRW",
    });
    format(price);
    taotalprice.innerText = formatted;
    taotalprice.innerText = price;
  };

  const claculate = () => {
    const result = selected.reduce((acc, cureent) => {
